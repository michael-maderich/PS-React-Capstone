...
const groceryItemsRouter = express.Router();
 ...
 */
groceryItemsRouter.get('/', (req, res) => { // so we can either get data from db in the beginning and rely
	// on react state for logic while keeping (updating) database in sync, or we can query database on every action
	GroceryItem		// mongoose model item based on declared schema
		.find({})		// Finds all
...
/**
   ...
   * @apiError (Bad Request 400)
 */
groceryItemsRouter.get('/:type', (req, res, next) => {
...
groceryItemsRouter.post('/', (req, res, next) => {
...
groceryItemsRouter.put('/:name', (req, res) => {
	const {body} = req;
...
groceryItemsRouter.delete('/:name', (req, res, next) => {
...
export default groceryItemsRouter;
